...
export const baseAccountSetupSchema = z.object({
  ...
  reward: z
    .string()
    .default('Access portfolio tracking and future rebalancing features'),
...
export const baseActionSchema = z.union([
  baseAccountSetupSchema,
  usdBcAcquisitionSchema,
]);
...
export const createBaseAction = (
  ...
  if (!overrides.title || overrides.title === 'Set up Base Account') {
    return createBaseAccountSetupAction(
      overrides as Partial<BaseAccountSetupAction>,
    );
  } else if (overrides.title === 'Get USDbC Stablecoins') {
    return createUsdBcAcquisitionAction(
      overrides as Partial<UsdBcAcquisitionAction>,
    );
  } else {
