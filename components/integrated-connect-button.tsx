...
export function IntegratedConnectButton() {
  ...
  const handleAuthenticate = async () => {
    ...
    try {
      ...
      console.log(`Initiating SIWE authentication for ${address} on ${currentHostname}`);
      ...
      else if (err.message?.includes("window") || err.message?.includes("popup")) {
        toast.error(
          "Failed to open signature window. Please check if pop-ups are blocked."
        );
      }
      // Handle domain mismatch issues
      else if (err.message?.includes("domain") || err.message?.includes("site")) {
        toast.error(
  ...
  const customButtonRender = useCallback(
    ({ show, isConnected, address, ensName }: any) => {
      ...
      const showAuthenticatingState = isConnected && !isAuthenticated && isAuthenticating;

      return (
        <button
          onClick={isConnected && !isAuthenticated ? handleAuthenticate : handleConnect}
    ...
    [isConnecting, isAuthenticating, isAuthenticated]
  );
