...
export async function GET(req: Request) {
  ...
  if (!sellToken || !buyToken || !sellAmount) {
    return NextResponse.json({ error: "sellToken, buyToken, and sellAmount required" }, { status: 400 });
  }

  const ZERO_EX_API_KEY = process.env.ZERO_EX_API_KEY!;
  if (!ZERO_EX_API_KEY) {
    return NextResponse.json({ error: "Swap API key not configured" }, { status: 500 });
  }
  ...
  if (!res.ok) {
    return NextResponse.json({ error: data.validationErrors?.[0]?.reason || data.reason || "Swap API error" }, { status: res.status });
  }
