...
export default function MyActionsPage() {
  ...
  const { userActions, isLoading: actionsLoading, completeAction } = useActions();
    ...
    return (
      <div className="container p-8 space-y-8">
        ...
        <div className="flex min-h-[400px] items-center justify-center">
          <p className="text-muted-foreground">Loading your actions and rewards...</p>
        </div>
      </div>
    );
  }

  const pendingRewards = userRewards.filter((reward) => !reward.claimed);
  const claimedRewards = userRewards.filter((reward) => reward.claimed);
    ...
    <div className="container p-8 space-y-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <Card>
          ...
          <CardContent>
            {userActions.filter((action) => action.status === "IN_PROGRESS").length === 0 ? (
                  ...
                  .map((userAction) => (
                    <li key={userAction.id} className="border rounded-lg p-4">
                      <div className="flex justify-between items-start mb-2">
                        <h3 className="font-medium">{userAction.action?.title}</h3>
                        <Badge variant="outline">{userAction.action?.difficulty}</Badge>
                      </div>
        ...
        <Card>
          ...
          <CardContent>
            {userActions.filter((action) => action.status === "COMPLETED").length === 0 ? (
