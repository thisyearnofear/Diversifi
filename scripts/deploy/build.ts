...
try {
  ...
  if (isNetlify) {
    ...
    execSync("pnpm add -D -w stream-browserify buffer crypto-browserify util", {
      stdio: "inherit",
    });
  }
  ...
  if (isNetlify) {
    ...
    if (process.env.NETLIFY_NEXT_PLUGIN_SKIP === "true") {
      ...
      try {
        ...
        execSync("cp -r .next/standalone/* .next/ 2>/dev/null || true", {
          stdio: "inherit",
        });
        execSync("cp -r .next/static .next/standalone/ 2>/dev/null || true", {
          stdio: "inherit",
        });
