...
function patchNobleHashes(installPath) {
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      console.warn(`Failed to patch package.json at ${packageJsonPath}:`, error.message);
    }
  }
...
//# sourceMappingURL=utils.js.map`
        ...
        console.warn(`Failed to patch ESM utils.js at ${esmUtilsPath}:`, error.message);
      }
    }
...
//# sourceMappingURL=utils.js.map`
      ...
      console.warn(`Failed to patch CommonJS utils.js at ${cjsUtilsPath}:`, error.message);
    }
  }
  ...
  if (!fs.existsSync(legacyPath)) {
    try {
      ...
      console.warn(`Failed to create legacy.js at ${legacyPath}:`, error.message);
    }
  } else {
  ...
  for (const file of compatFiles) {
    ...
    if (!fs.existsSync(filePath)) {
      try {
        ...
        console.warn(`Failed to create ${file.name} at ${filePath}:`, error.message);
      }
    }
  }
}
...
try {
  ...
  if (installations.length === 0) {
    console.log('No @noble/hashes installations found, skipping patching');
    process.exit(0);
  }
