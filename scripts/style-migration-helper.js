...
function scanFile(filePath) {
  try {
    ...
    const hasStyleImports =
      content.includes('getRegionStyle') ||
...
function generateClassNameSuggestion(lineText, matches) {
  ...
  if (hasRegionColor) {
    ...
    suggestion = suggestion.replace(
      /className="([^"]*)"/,
      `className={getRegionStyle("${region}", "medium", "bg")}`,
    );
  } else if (hasChainColor) {
    const chain = matches.find((m) => m.chain)?.chain || 'BASE';
    suggestion = suggestion.replace(
      /className="([^"]*)"/,
      `className={getChainStyle("${chain}", "medium", "bg")}`,
    );
  }
...
function generateCnSuggestion(lineText, matches) {
  ...
  if (hasRegionColor) {
    ...
    suggestion = suggestion.replace(
      /cn\(([^)]*)\)/,
      `cn($1, getRegionStyle("${region}", "medium", "bg"))`,
    );
  } else if (hasChainColor) {
    const chain = matches.find((m) => m.chain)?.chain || 'BASE';
    suggestion = suggestion.replace(
      /cn\(([^)]*)\)/,
      `cn($1, getChainStyle("${chain}", "medium", "bg"))`,
    );
  }
...
function main() {
  ...
  const allResults = [...componentsResults, ...appResults].sort(
  ...
  allResults.slice(0, 10).forEach((result, index) => {
    console.log(
      `${index + 1}. ${result.path} (${result.totalMatches} matches)`,
    );
