...
export function usePolygonDivviRegistration() {
  ...
  const { data: registrationStatus, refetch: refetchRegistrationStatus } =
    useReadContract({
      ...
      args: address
        ? [address, [stringToHex('allbridge', { size: 32 })]]
        : undefined,
      chainId: polygon.id,
    });
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        'Registration status:',
        isUserRegistered ? 'already-registered' : 'not-registered',
      );
    }
  }, [registrationStatus]);
  ...
  useEffect(() => {
    const checkRegistrationStatus = async () => {
      ...
      try {
        ...
        console.error(
          'Error checking Polygon Divvi registration status:',
          error,
        );
        setStatus('error');
        setError(
  ...
  useEffect(() => {
    if (writeContractError) {
      ...
      if (
        ...
        setError(
          "Transaction simulation failed. This could be because you're already registered or there's an issue with the protocol ID.",
        );
      } else {
  ...
  const checkReferrerRegistration = async (
    referrerId: string,
    protocolId: string,
  ) => {
    try {
      ...
      console.log(
        `Assuming referrer ${referrerId} is registered for protocol ${protocolId}`,
      );
      return true;
    } catch (error) {
  ...
  const register = async () => {
    ...
    try {
      ...
      if (
        ...
        Array.isArray(isRegisteredResult.data) &&
        isRegisteredResult.data[0]
      ) {
      ...
      const isReferrerRegistered = await checkReferrerRegistration(
        referrerId,
        protocolId,
      );
      ...
      if (!isReferrerRegistered) {
        setError(
          `Referrer ${referrerId} is not registered for protocol ${protocolId}. Please try a different protocol.`,
        );
        setStatus('error');
        return;
      }

      const protocolsToRegister = [stringToHex(protocolId, { size: 32 })];
      ...
      try {
        ...
        if (!actionResponse.ok) {
          ...
          if (actionResponse.status === 404) {
            ...
            console.warn(
              `Error ${actionResponse.status} getting action ID, continuing anyway`,
            );
          }
        } else {
