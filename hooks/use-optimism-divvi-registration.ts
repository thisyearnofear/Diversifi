...
export function useOptimismDivviRegistration() {
  ...
  const {
    ...
    args: address
      ? [address, [stringToHex('velodrome', { size: 32 })]]
      : undefined,
    chainId: optimism.id,
  });
  ...
  useEffect(() => {
    ...
    if (isCheckingRegistration) {
      ...
      registrationStatus &&
      (registrationStatus as unknown as boolean[])?.[0]
    ) {
  ...
  const register = async () => {
    ...
    try {
      ...
      if (
        ...
        (isRegisteredResult.data as unknown as boolean[])?.[0]
      ) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (response.status === 404) {
            ...
            console.debug(
              "Action 'Register on Optimism' not found in database, marking as completed anyway",
            );
            setStatus('completed');
