...
const fetchTokenBalance = async (token: string, address: string): Promise<string> => {
  if (!address) {
...
export function useTokenBalancesQuery(selectedRegion?: string) {
  ...
  const refreshBalances = () => {
    return queryClient.invalidateQueries({ queryKey: ['tokenBalances', address, selectedRegion] });
  };
