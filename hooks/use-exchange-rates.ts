...
export function useExchangeRates() {
  ...
  const calculateExchangeRate = () => {
    ...
    if (prices?.cUSD?.usd && prices?.cKES?.usd) {
      ...
      console.log("Calculated exchange rate from direct token prices:", directRate);
      return directRate;
    }
    ...
    if (prices?.cUSD?.usd && rates && rates["USD/KES"]) {
      ...
      console.log("Calculated exchange rate from cUSD/USD and USD/KES:", calculatedRate);
  ...
  const fetchExchangeRate = async () => {
    try {
      ...
      const rate = parseFloat(ethers.utils.formatUnits(quoteAmountOut, 18));
      ...
      console.warn("Error fetching rate from Mento SDK, falling back to API data:", sdkError);
